...
| `update_source`                     | Bind (or unbind) an extraction schema and/or set `publisher_id` on a watched source. Pass `reprocess: true` to fill-missing re-extract prior versions. Requires `policy-update:manage`. |
